...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-semibold text-zinc-900 mt-2">New Entry</h1>
</div>

<form method="POST" action="{{ route('entries.store') }}" class="space-y-5">
    @csrf
            ...
            <div>
                ...
                >
                    ...
                    @foreach($entryTypes as $entryType)
                        <option value="{{ $entryType->id }}" @selected(old('type_id') == $entryType->id)>
                            {{ $entryType->name }}
                        </option>
                    @endforeach
                </select>
                ...
                @if($entryTypes->isEmpty())
                    <p class="mt-1.5 text-xs text-zinc-400">No types yet. <a href="{{ route('entry-types.create') }}" class="text-indigo-600 hover:text-indigo-700">Create one first.</a></p>
                @endif
            </div>
                    ...
                    @foreach($topics as $topic)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                name="topics[]"
                                value="{{ $topic->id }}"
                                @checked(in_array($topic->id, old('topics', [])))
        ...
        <div class="grid grid-cols-2 divide-x divide-zinc-200" style="min-height: 400px">
            <div class="flex flex-col">
                ...
                <textarea
                    ...
                    class="flex-1 w-full px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 font-mono resize-none focus:outline-none @error('content') border border-red-500 rounded-b-xl @endif"
