    ...
    <header class="border-b border-zinc-200 bg-white">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
            ...
            <nav class="flex items-center gap-3">
                @if (Route::has('login'))
                    <a href="{{ route('login') }}" class="text-sm text-zinc-600 hover:text-zinc-900 font-medium transition-colors">
                        Sign in
                    </a>
                @endif
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-3 py-1.5 rounded-lg transition-colors">
                        Get started
                    </a>
                @endif
            </nav>
